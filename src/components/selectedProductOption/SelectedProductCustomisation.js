...
function SelectedProductCustomisation({ customizeProduct, handleOk, coustomizeProductID }) {
    ...
    useEffect(() => {
        if(productState.savedProduct){
            ...
            productState.savedProduct.map((eachItem,saveIndex) => {
                if(eachItem.index===coustomizeProductID.productIndex && eachItem.productID===coustomizeProductID.productID){
                    flag=true;
                            ...
                            productAttr.product_color_image.map((item,colorIndex)=>{
                                if(eachItem.attributecolor===item.color_name){
                                    setSaveProduct({
                                        ...
                                        productName:productState.displayProduct[coustomizeProductID.productIndex].product_name,
                                        price:productState.displayProduct[coustomizeProductID.productIndex].price,
                                        totalPrice:productState.displayProduct[coustomizeProductID.productIndex].total_price,
    ...
    useEffect(() => {
        ...
        if(!flag){
            tempSelect.attribute_Image = selectedItem.product_color_image[0].product_image;
    ...
    const onClickProduct = (e) => {
        ...
        let flag=false;
        customizeProduct[e.target.id].product_color_image.map((eachColorObj, index) => {
            if(eachColorObj.product_image===selectedItem.attribute_Image){
                flag=true;
            }
        })
        if(!flag){
            tempProduct.attribute_Image = customizeProduct[e.target.id].product_color_image[0].product_image;
        }
            ...
            <div className="slected-area-top">
                    ...
                    <div className="color-values">
                        {
                            selectedItem.product_color_image.map((eachColorObj, index) => (
                                ...
                                <div className={eachColorObj.product_image===selectedItem.attribute_Image?"each-color active":"each-color"} key={index}>
                ...
                <div className="product-description">
                    ...
                    <p className="description-text">
                        {productState.displayProduct[coustomizeProductID.productIndex].short_descript}
                    </p>
                </div>
            </div>
            <div className="selected-area-bottom">
                <div className="product-name">
                    <div className="name-text">
                        <b>{productState.displayProduct[coustomizeProductID.productIndex].product_name}</b>
                    </div>
                    <div className="product-type">
                        {selectedItem.attribute_value[0]}
                    </div>
                </div>
                <div
                    className="ok-button"
